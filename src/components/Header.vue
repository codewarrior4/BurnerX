<script setup>
import { Menu, CheckCircle2, Copy, RefreshCw, Bell, BellOff } from 'lucide-vue-next'
import { 
  currentAccount, copyToClipboard, copyStatus, isPolling, 
  fetchMessages, isSidebarOpen, notificationsEnabled, requestNotificationPermission
  ...
  <header class="h-20 border-b border-burner-border flex items-center justify-between px-4 lg:px-8 bg-burner-dark/80 backdrop-blur-md z-30">
    ...
    <div class="flex items-center gap-2 shrink-0">
      ...
      <button 
        ...
        :class="['p-2 transition-colors rounded-lg', notificationsEnabled ? 'text-burner-accent bg-burner-accent/10' : 'text-burner-text-dim hover:text-burner-accent']"
        :title="notificationsEnabled ? 'Notifications On' : 'Enable Notifications'"
      >
        <Bell v-if="notificationsEnabled" class="w-5 h-5" />
